...
export function MediaGeneratorPage({ kind, title, subtitle, emoji, promptPlaceholder, templatePrompt, fields }: Props) {
  ...
  return (
    <>
      <Head>
        <title>{pageTitle} - EcomPic AI</title>
      ...
      <main className="min-h-screen px-4 py-6 md:py-10">
        <div className="mx-auto max-w-6xl">
          <header className="mb-6 flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:flex-row md:items-center md:justify-between">
            <Link href="/" className="inline-flex h-10 items-center text-sm font-semibold text-slate-500 hover:text-brand-600">← {tr.backHome}</Link>
            <div className="flex flex-wrap items-center gap-2"><nav className="flex flex-wrap gap-2" aria-label={tr.mediaNav}>{toolLinks(kind, tr)}</nav><button onClick={toggleLang} className="inline-flex h-10 items-center rounded-xl border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-500 hover:border-brand-400">{lang === 'zh' ? 'EN' : '中'}</button></div>
          </header>
          ...
          <section className="mb-6 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            <div className="grid gap-0 md:grid-cols-[1.05fr_.95fr]">
              <div className="p-7 md:p-9">
                ...
                <h1 className="text-3xl font-black leading-tight text-slate-900 md:text-5xl">{pageTitle}</h1>
                <p className="mt-4 max-w-2xl text-sm leading-7 text-slate-500 md:text-base">{pageSubtitle}</p>
              </div>
              <div className="border-t border-slate-200 bg-slate-50 p-7 md:border-l md:border-t-0 md:p-9">
                <h2 className="text-sm font-bold text-slate-900">{tr.singleFunctionPage}</h2>
                <p className="mt-2 text-sm leading-6 text-slate-500">{tr.singleFunctionDesc}</p>
                <div className="mt-5 grid gap-2 text-xs text-slate-500">
                  {fields.slice(0, 4).map((f, idx) => { const ft = fieldText(f); return <div key={idx} className="rounded-xl bg-white px-3 py-2 shadow-sm"><span className="font-semibold text-slate-700">{ft.name}</span> · {ft.description}</div>; })}
                </div>
              </div>
            </div>
          </section>
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
            <section className="space-y-5 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-6">
              <div>
                <label className="block text-sm font-bold text-slate-800">{tr.mediaPromptLabel}</label>
                <p className="mt-1 text-xs leading-5 text-slate-500">{tr.mediaPromptHelp}</p>
              ...
              <div className="grid gap-4 md:grid-cols-3">
                {fields.some(f => f.control === 'style') && <div className="rounded-2xl bg-slate-50 p-4"><label className="text-sm font-bold text-slate-800">{tr.style}</label><p className="mt-1 min-h-[40px] text-xs leading-5 text-slate-500">{tr.styleDesc}</p><input value={style} onChange={e => setStyle(e.target.value)} className="mt-3 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none" placeholder={tr.stylePlaceholder} /></div>}
                {fields.some(f => f.control === 'duration') && <div className="rounded-2xl bg-slate-50 p-4"><label className="text-sm font-bold text-slate-800">{tr.duration}</label><p className="mt-1 min-h-[40px] text-xs leading-5 text-slate-500">{tr.durationDesc}</p><input type="number" min={3} max={60} value={duration} onChange={e => setDuration(Number(e.target.value || 5))} className="mt-3 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none" /></div>}
                {fields.some(f => f.control === 'aspectRatio') && <div className="rounded-2xl bg-slate-50 p-4"><label className="text-sm font-bold text-slate-800">{tr.aspectRatio}</label><p className="mt-1 min-h-[40px] text-xs leading-5 text-slate-500">{tr.aspectRatioDesc}</p><select value={aspectRatio} onChange={e => setAspectRatio(e.target.value)} className="mt-3 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none"><option>16:9</option><option>9:16</option><option>1:1</option></select></div>}
              </div>
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                <button type="button" onClick={() => setPrompt(templatePrompt)} className="inline-flex min-h-[48px] items-center justify-center rounded-xl bg-slate-100 px-5 py-3 font-bold text-slate-700 hover:bg-slate-200">{tr.applyTemplate}</button>
              </div>
            </section>

            <aside className="space-y-5">
              <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                <h2 className="text-base font-black text-slate-900">{tr.parameterGuide}</h2>
                <div className="mt-4 space-y-3">
                  {fields.map((f, idx) => { const ft = fieldText(f); return <div key={idx} className="rounded-2xl bg-slate-50 p-4"><div className="text-sm font-bold text-slate-800">{ft.name}</div><div className="mt-1 text-xs leading-5 text-slate-500">{ft.description}</div></div>; })}
                </div>
              </div>
              <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                <h2 className="text-base font-black text-slate-900">{tr.generatedResult}</h2>
                {!result && <div className="mt-4 rounded-2xl bg-slate-50 p-6 text-center text-sm leading-6 text-slate-500">{tr.resultPlaceholder}</div>}
                ...
                {result && <a href={result} download={downloadName(kind)} className="mt-4 inline-flex min-h-[40px] items-center rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white">⬇️ {tr.downloadResult}</a>}
              </div>
            </aside>
          </div>
        </div>
